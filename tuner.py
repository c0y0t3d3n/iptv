...
class HDHR():
    ...
    def run(self):
        ...
        def stream(stream):
            global ACCOUNTS,SOURCES,LINEUP,PROCS,FORMAT,DIRECT,CMD
            if LINEUP and stream in LINEUP:
                logging.info('%s stream %s'%(request.remote_addr,stream))
                l=LINEUP[stream]
                SOURCES=refresh_accts(ACCOUNTS)
                ...
                if int(DIRECT):
                    ...
                    #logging.info('%s redirect to %s', request.remote_addr, url)
                    return redirtect(url)
                else:
                    # remux with ffmpeg
                    args = CMD % url
                    logging.info('%s start %s', request.remote_addr, args)
                    try:
                        cmd = subprocess.Popen(args.split(), shell=False, stdout=subprocess.PIPE)
                        logging.info('%s pid %s', request.remote_addr, cmd.pid)
                        PROCS[cmd.pid]=(request.remote_addr,args)
                    except Exception as e:
                        logging.exception(e)
                        return str(e),500
                    ...
                    def generate():
                        ...
                        cmd.wait()
                        logging.info('%s pid %s stop (%d)', request.remote_addr, cmd.pid, cmd.returncode)
                        del PROCS[cmd.pid]
        ...
        def discover():
                global SERVER_IP,SERVER_PORT,TUNER_COUNT
                return json.dumps({
                    "DeviceID": "TUNER",
                    "FriendlyName": "Tuner",
        ...
        def lineup_json():
                return json.dumps(list(LINEUP.values()))

        @app.route('/lineup')
        def lineup():
            global LINEUP,SOURCE_GROUPS
        ...
        @app.route('/')
        def status():
            global PROCS, CONFIG_FILE, SOURCES, LINEUP
            html=self.html_start()
            try:
                if PROCS:
                ...
                if SOURCES:
                    ...
                    html+='''<tr><td><form action='/reload' method=post><input type=submit name=refresh value=refresh></form></td></tr></table></p>'''
                html+='''<p><table>'''
        ...
        global SERVER_IP, SERVER_PORT, LOGLEVEL
        app.run(host=SERVER_IP, port=int(SERVER_PORT), debug=int(LOGLEVEL)==10)
...
<body>
    ...
    def html_end(self):
        ...
        if CONFIG_FILE:
            html+='''
            <p>&nbsp;</p><p><form action='/config' method=post>
...
def main(*args):
    ...
    globals().update(Flask=Flask,redirect=redirect,request=request, stream_with_context=stream_with_context)
